...
void write(){
	int a = rand() % SIZE_DATA;
	printf("I updated data's %d Item %d to\n", a, Key_vector[a]);
	Key_vector[a] = rand() % SIZE_DATA;
	printf("the new data %d\n", Key_vector[a]);
}
...
void transaction(int thread_num){
	printf("----Txs (%d)-----\n", thread_num);
